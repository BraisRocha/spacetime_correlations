...
grid cell. Each job:
    ...
    1. Generates ``n_simulations`` isotropic background realizations.
    2. Injects a flare of the requested duration and signal-to-noise
       ratio on top of each parent sample.
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=float,
        required=True,
        help="Flare signal-to-noise ratio (mean flare events / mu_window).",
    )
    parser.add_argument(
        "--seed",
        type=int,
...
def main(
    ...
    # Flare parameters (single point)
    flare_duration = flare_duration_days * u.day
    flare_sigma = 1.0  # deg
    flare_intensity = float(flare_intensity_value)  # Signal to Noise ratio
    ...
    while n_success < n_simulations and attempt < max_attempts:
        ...
        try:
            ...
            if n_flare == 0:
                ...
                event_sample.inject_flare(flare=flare)

            subsample = event_sample.select_subsample(window=window)
    ...
    if n_success < n_simulations:
        logger.warning(
            ...
            "before reaching max_attempts=%d. Trimming Lambda array.",
            n_simulations,
            n_success,
            max_attempts,
        )
